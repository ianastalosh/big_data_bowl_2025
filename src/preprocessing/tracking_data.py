...
class TrackingDataProcessor:
    ...
    def _normalise_features(self) -> pl.DataFrame:
        ...
        tracking_data = tracking_data.with_columns(
            pl.when(pl.col("playDirection") == "left").then(Constants.X_MAX - pl.col("x")).otherwise(pl.col("x")).alias("x"),
            pl.when(pl.col("playDirection") == "left").then(Constants.Y_MAX - pl.col("y")).otherwise(pl.col("y")).alias("y"),
